    ...
    */
    public function modificar(){
        ...
        $consulta = "UPDATE pasajero 
                    ...
                    idviaje = ".$this->getObjViaje()->getIdViaje()." WHERE rdocumento = ".$this->getDocumento();
        if($baseDatos->iniciar()){
            if($baseDatos->ejecutar($consulta)){
                $resp = true;
            }else{
		...
		if($baseDatos->iniciar()){
			if($baseDatos->ejecutar($consultaPasajero)){
				...
				while($pasajero=$baseDatos->registro()){	
					...
					$objPasajero->buscar($pasajero['rdocumento']); //arma el objeto pasajero
                    array_push($resp, $objPasajero); //lo agrega al array y lo retorna
				}
		 	}else{
                $resp = false;
                $this->setMensajeError($baseDatos->getERROR());
